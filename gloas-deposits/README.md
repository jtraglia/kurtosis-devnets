...
| s5 | Post  | Submits 50,000 invalid validator deposits for a single pubkey, then one valid builder deposit for that pubkey. |
| s6 | Post  | Submits 50,000 invalid validator deposits for a single pubkey, then one valid validator deposit for that pubkey, then one valid builder deposit for that pubkey. |
| s7 | Pre   | Submits 50,000 invalid validator deposits for a single pubkey, then one valid builder deposit for that pubkey. |
